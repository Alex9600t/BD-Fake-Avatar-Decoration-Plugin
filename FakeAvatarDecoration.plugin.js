...
function vc_fad_input() {
    ...
    if (data && data !== "") {
        BdApi.Net.fetch("https://vcfad.vercel.app/setAvatarDecoration", {
            method: "POST",
            headers: {
...
function settings() {
    ...
    return(`${blockH2("Update Avatar Decoration")}${blockInput(blockSub(`Enter ID. If you don’t know it, you can <a onclick='window.open("https://vcfad.vercel.app/list", "_blank")'>view all IDs</a>`), "")} <div style="border-bottom: 1px solid; margin-bottom: 20px;"></div> ${blockBtn("Publish", () => {console.log("Hey")})}`)
}
